...
describe("packageMetadata", () => {
	...
	it("forwards to api.wally.run with a compliant User-Agent", async () => {
		...
		await makeApp().fetch(studioRequest("http://localhost/v1/package-metadata/jsdotlua/react"));

		const { url, headers } = getFetchArgs(fetchMock);
		expect(url.toString()).toBe("https://api.wally.run/v1/package-metadata/jsdotlua/react");
	...
	it("returns 400 when scope contains uppercase letters", async () => {
		const response = await makeApp().fetch(studioRequest("http://localhost/v1/package-metadata/JsDotLua/react"));
		expect(response.status).toBe(400);
	});

	it("returns 400 when name contains disallowed characters", async () => {
		const response = await makeApp().fetch(studioRequest("http://localhost/v1/package-metadata/jsdotlua/react_core"));
		expect(response.status).toBe(400);
	});
